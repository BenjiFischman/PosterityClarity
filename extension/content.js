function captureProfileUrls() {
	...
	//literally hardcoded the encoded bundles
	...
	// send profile urls back to background for proccesing
...
function do_Work()  {
	...
	//does not ensure url location is on about, will not fix
...
if (profile_location == '_list') {
		...
		} else if (profile_location === 'about' && !living.test(currentUrl) && !education.test(currentUrl)
			&& !contact.test(currentUrl)){
...
chrome.runtime.onMessage.addListener(
	...
	function(request, sender, sendReponse) {
		...
		else if ( request.message === "do_it" || ( request.message === "scrape_profile_info")) {
		do_Work();
	}}
