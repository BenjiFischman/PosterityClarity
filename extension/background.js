var profileUrlCollection = ['https://www.facebook.com/bfischman1?fref=grp_mmbr_list'];
	...
	function(request, sender, sendResponse) {
		if (request.message === 'store_profile_urls') {
			...
			sendMessageToActiveTab({
		 	...
		 	sendMessageToActiveTab({'message' : 'do_it'});
		}
		...
		} else if (request.message === 'home_info' && incomplete_Profile[0]) {
			if (profileUrlCollection.length !== 0) {
				if (request.home_Info == undefined || request.home_Info == null){
				...
				//var place_holder = start_Next_Scrape();
